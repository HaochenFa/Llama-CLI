...
describe('ToolDispatcher', () => {
  ...
  describe('Tool Registration', () => {
    it('should register native tools automatically', () => {
    ...
    it('should filter out native tools from additional tools', () => {
      ...
      // Should only have one read_file tool (the original native one)
  ...
  describe('Tool Dispatch', () => {
    ...
    it('should handle tool execution error', async () => {
      // Mock the read_file tool to throw an error
      const { read_file_tool } = await import('../../src/lib/tools/read_file');
      (read_file_tool.invoke as jest.Mock).mockRejectedValueOnce(new Error('File not found'));

      const toolCall = {
        name: 'read_file',
        arguments: { file_path: '/non/existent/file.txt' }
      };

      const result = await toolDispatcher.dispatch(toolCall, 'call-123');

      expect(result.role).toBe('tool');
      expect(result.content).toContain("Error executing native tool 'read_file': File not found");
      expect(result.tool_call_id).toBe('call-123');
    });
  ...
  describe('Tool Information', () => {
    ...
    it('should have proper parameter schemas', () => {
      ...
      expect(readFileTool?.parameters.required).toContain('file_path');
    });
  });
});
