...
describe("FileContextManager", () => {
  ...
  describe("loadFiles", () => {
    it("should load multiple files successfully", async () => {
      ...
      mockFs.readFileSync.mockReturnValueOnce(fileContents[0]).mockReturnValueOnce(fileContents[1]);
  ...
  describe("isFileInContext", () => {
    it("should detect file in context", () => {
      const filePath = "test.txt";
      const absolutePath = path.resolve(testWorkingDir, filePath);
      const context: FileContext[] = [{ path: absolutePath, content: "content" }];
    ...
    it("should detect file not in context", () => {
      const filePath = "test.txt";
      const context: FileContext[] = [{ path: "/other/path/file.txt", content: "content" }];
  ...
  describe("isSupportedFileType", () => {
    ...
    it("should support configuration files", () => {
      const configFiles = ["package.json", "config.yaml", "settings.yml", "data.xml", "README.md"];
    ...
    it("should not support binary files", () => {
      const binaryFiles = ["image.png", "video.mp4", "archive.zip", "executable.exe", "library.so"];
