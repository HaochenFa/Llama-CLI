...
export const mcp_manager_tool: ToolDefinition = {
  type: 'native',
  name: 'mcp_manager',
  description: 'Manage Model Context Protocol (MCP) servers and connections. Supports listing, connecting, disconnecting, and getting information about MCP servers and their tools.',
  parameters: {
    type: 'object',
    properties: {
      action: {
        type: 'string',
        enum: ['list', 'connect', 'disconnect', 'status', 'tools', 'resources', 'test', 'add', 'remove', 'enable', 'disable'],
        description: 'The action to perform: list (show servers), connect (connect to server), disconnect (disconnect from server), status (show connection status), tools (list available tools), resources (list available resources), test (test connections), add (add new server), remove (remove server), enable/disable (enable/disable server)'
      },
      server_id: {
        type: 'string',
        description: 'Server ID for actions that target a specific server (connect, disconnect, remove, enable, disable)'
      },
      server_name: {
        type: 'string',
