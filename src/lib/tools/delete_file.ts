...
export const delete_file_tool: ToolDefinition = {
  type: 'native',
  name: 'delete_file',
  description: 'Deletes a specified file from the local filesystem. This is a destructive operation that requires user confirmation. The file path must be an absolute path.',
  parameters: {
    type: 'object',
    properties: {
      file_path: {
      ...
      force: {
        type: 'boolean',
        description: "Optional: Skip confirmation prompt if set to true. Use with extreme caution. Default: false.",
        default: false
      }
    },
    required: ['file_path'],
  ...
  invoke: async (args: { file_path: string; force?: boolean }) => {
    ...
    try {
      ...
      const isDangerous = dangerousPatterns.some(pattern => pattern.test(fileName) || pattern.test(file_path));
