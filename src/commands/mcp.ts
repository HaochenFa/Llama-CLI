    ...
    .action(async (options: { connectedOnly: boolean }) => {
      try {
        ...
        for (const server of servers) {
          ...
          console.log(
            `  Auto-connect: ${server.autoConnect ? chalk.green("Yes") : chalk.gray("No")}`
          );
    ...
    .action(
      async (options: {
        ...
        try {
          ...
          console.log(
            chalk.green(`✅ MCP server "${options.name}" added successfully with ID: ${serverId}`)
          );
  ...
  mcpCommand
    ...
    .action(async (serverId: string) => {
      try {
        ...
        console.error(
          chalk.red(`❌ Error disconnecting from MCP server: ${(error as Error).message}`)
        );
        process.exit(1);
      }
    });
